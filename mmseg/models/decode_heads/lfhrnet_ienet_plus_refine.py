...
class LFHRIENETPLUSREFINE(BaseDecodeHead):
    def __init__(self, sai_number, lf_number, key_channels, value_channels, warp_channels, disp_channels, dis_candidate, valid_dis_range, **kwargs):
        super(LFHRIENETPLUSREFINE, self).__init__(**kwargs)
    ...
    def forward(self, inputs, seg_logits, sequence_imgs, sequence_warp_imgs, lf_imgs, sequence_index):
        ...
        _, _, H, W = center_encoding_feature.shape
        ...
        seg_logits = resize(
                seg_logits,
                size=center_encoding_feature.shape[2:],
                mode='bilinear',
                warning=False)          
        ...
        disparity_4 = disparity_4.permute(0, 2, 3, 1).contiguous()     
