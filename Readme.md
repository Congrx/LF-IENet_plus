...
- `work_dirs`：Our pre-trained models  `LF-IENet++`$^{4}$`-Res50` and `LF-IENet++`$^{3}$`-Res101` and `LF-IENet++`$^{3}$`-HR48` with default configuration information on `UrbanLF-Real` and `UrbanLF-Syn-Big-dis` and `UrbanLF-Syn-Small-dis`. 
